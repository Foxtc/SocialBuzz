...
def collect_all_social_searchs():
    social_search_list = FacebookPage.objects.all()
    for social_search in social_search_list:
        collect_social_search.apply_async(args=[social_search.page_id])

@task(ignore_result=True)
def collect_social_search(social_search_id):
    try:
        social_search = FacebookPage.objects.get(page_id=social_search_id)
